...
/*
  OT Rounding নিয়ম (প্রতিটি ঘন্টার অতিরিক্ত মিনিটের জন্য):
  0 - 24 min  →  0    (count নেই)
  25 - 54 min →  0.5  (আধা ঘন্টা)
  55+ min     →  1    (পুরো ঘন্টা)

  উদাহরণ:
  20min → 0 | 30min → 0.5 | 55min → 1
  1h20m → 1 | 1h30m → 1.5 | 1h55m → 2
  2h25m → 2.5 | 3h0m → 3
*/
...
/* Format time: "06:52 AM" বা "06:52:48 AM" → সবসময় "H:MM:SS AM" format */
function formatTimeExcel(t) {
  if (!t || !t.trim()) return "";
  t = t.trim();
  const ampm = t.match(/(AM|PM)/i);
  if (!ampm) return t;
...
function processPaste() {
  ...
  for (const line of lines) {
    ...
    /* Team column detection — short uppercase code like ICS, TLS, LHG, ATS, QC */
    const TEAM_PAT = /^[A-Z]{2,6}$/;
    let designation = "—", team = "—";
    if (cols[2] && TEAM_PAT.test(cols[2])) {
      team = cols[2];
      designation = cols[3] || "—";
    } else if (cols[3] && TEAM_PAT.test(cols[3])) {
      designation = cols[2] || "—";
      team = cols[3];
    } else {
      designation = cols[2] || "—";
    ...
    for (let i = 3; i < cols.length; i++) {
      ...
      if (/^present$/i.test(c))      status = "Present";
        ...
        /* keep original format including seconds */
        if (!inTime) inTime = c; else if (!outTime) outTime = c;
      }
    }
...
- id (string)
- designation (string)
- team (string — e.g. "ICS","TLS","LHG","ATS", or "" if absent)
- shift: "Morning" or "Evening"
- status: "Present" or "Day Off"
- inTime (string e.g. "8:45:00 AM", keep seconds if visible, or "")
- outTime (string e.g. "8:35:00 PM", keep seconds if visible, or "")
JSON array only, no markdown.` }
          ]}]
        }),
      });
      const data = await resp.json();
      const rows = JSON.parse(data.content.map(i=>i.text||"").join("").replace(/```json|```/g,"").trim());
      let added = 0;
      for (const row of rows) {
        const otMins = (row.status==="Present" && row.inTime && row.outTime) ? calcOT(row.inTime, row.outTime) : 0;
      ...
      console.error(err);
      showStatus("⚠ AI পড়তে ব্যর্থ।", "red");
    }
  }
...
function addManualRow() {
  ...
  const inTime=fmt(inRaw), outTime=fmt(ouRaw);
...
function renderPreview() {
  ...
  body.innerHTML=employees.map((e,i)=>{
    ...
    return `<tr>
      ...
      <td>${shBadge}</td><td>${stBadge}</td>
      <td class="time-cell">${e.inTime||"—"}</td>
      <td class="time-cell">${e.outTime||"—"}</td>
      <td>${otBadge}</td>
      <td><button class="btn-del-row" onclick="deleteRow(${i})">
...
function renderReport() {
  ...
  body.innerHTML=sorted.map((e,i)=>{
    ...
    return `<tr>
      ...
      <td class="id-cell">${e.id}</td>
      <td class="id-cell">${e.team||"—"}</td>
      <td class="time-cell">${formatTimeExcel(e.inTime)||"—"}</td>
      <td class="time-cell">${formatTimeExcel(e.outTime)||"—"}</td>
      <td>
...
function copyTableForExcel() {
  ...
  const rows=sorted.map((e,i)=>[
    ...
    e.id,
    e.team||"—",
    formatTimeExcel(e.inTime)||"",
    formatTimeExcel(e.outTime)||"",
    minsToRoundedHours(e.otMins),
  ].join("\t"));
  ...
  navigator.clipboard.writeText(tsv).then(doFeedback).catch(()=>{
...
function exportCSV() {
  ...
  const lines=[
    ...
    ...sorted.map((e,i)=>
      `${i+1},"${e.name}","${e.id}","${e.team||""}","${formatTimeExcel(e.inTime)}","${formatTimeExcel(e.outTime)}","${minsToRoundedHours(e.otMins)}"`
    ),
  ];
  const a=document.createElement("a");
  a.href=URL.createObjectURL(new Blob([lines.join("\n")],{type:"text/csv;charset=utf-8;"}));
  a.download=`OT_Report_${date}.csv`; a.click();
}
